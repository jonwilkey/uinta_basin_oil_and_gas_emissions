...
well <- subset(well, subset = (drill_yr >= (min(eia.hp$year)-1) &
...
prior <- drill[(1:(nrow(drill)-1)),2]
wells <- drill[(2:nrow(drill)),2]


# Fit drilling model ------------------------------------------------------

# Create data.frame with all the data need to run lm()
analysis <- data.frame(eia.hp[,1], wells, prior, eia.hp[,c(2,3)])
