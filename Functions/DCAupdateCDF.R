...
DCAupdateCDF <- function(field, ver, DCA.CDF.type, cdf.oil.from, cdf.oil.to,
  ...
  mo <- subset(mo, subset = (qo.1 <=   opt$cdf.oil.to[1] &
                             b.1 <=    opt$cdf.oil.to[2] &
                             Di.1 <=   opt$cdf.oil.to[3] &
                             tdelay <= opt$cdf.oil.to[4]))
  
  mg <- subset(mg, subset = (qo.1 <=   opt$cdf.gas.to[1] &
                             b.1 <=    opt$cdf.gas.to[2] &
                             Di.1 <=   opt$cdf.gas.to[3] &
                             tdelay <= opt$cdf.gas.to[4]))
