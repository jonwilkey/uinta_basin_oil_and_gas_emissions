...
# eia.hp - data.frame containing historical energy prices in the Uinta Basin
...
EIAforecastUpdate <- function(forecast, basis, EIAbasis, tsteps, eia.hp, tstart,
                              ver, path, type) {
  ...
  # Get prices prior to start of simulation period
  ...
  opFPP <- zoo(tail(price$OP, 1), tail(as.Date(price$month), 1))
  gpFPP <- zoo(tail(price$GP, 1), tail(as.Date(price$month), 1))
  ...
  # Next, make zoo object for from forecast matrix for both oil and gas
  # forecasts
