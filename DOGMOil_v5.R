...
for (k in 1:length(fields)) {
  ...
  m1_R2 <- rbind(m1_R2,summary(m1_fit)$r.squared)
  ...
  m2_R2 <- rbind(m2_R2,summary(m2_fit)$r.squared)
  ...
  m4_R2 <- rbind(m4_R2,summary(m4_fit)$r.squared)
  ...
  m5_R2 <- rbind(m5_R2,summary(m5_fit)$r.squared)
  ...
  m6_R2 <- rbind(m6_R2,summary(m6_fit)$r.squared)
  ...
  m7_R2 <- rbind(m7_R2,summary(m7_fit)$r.squared)
  m7_p <- rbind(m7_p,summary(m7_fit)$coefficients[,4])
}

# Model 3: Basin aggregate
m3_fit <- lm(compare$APDs_approved ~ compare$oil_price + compare$lag_apd)
m3_coef <- coef(m3_fit)
m3_R2 <- summary(m3_fit)$r.squared
m3_p <- summary(m3_fit)$coefficients[,4]

# Model 8: Basin aggregate with gas
m8_fit <- lm(compare$APDs_approved ~ compare$oil_price + compare$gas_price + compare$lag_apd)
m8_coef <- coef(m8_fit)
m8_R2 <- summary(m8_fit)$r.squared
...
# Model 1 Predictions
m1_pred <- matrix(0, nrow = length(compare$apd_date), ncol = length(fields))
for (k in 1:length(fields)) {
  Wo <- 0
 for (j in 1:length(compare$apd_date)) {
...
# Model 2 Predictions
m2_pred <- matrix(0, nrow = length(compare$apd_date), ncol = length(fields))
for (k in 1:length(fields)) {
  a <- m2_coef[k,1]
  b <- m2_coef[k,2]
  c <- m2_coef[k,3]
...
# Model 3 Predictions
m3_pred <- matrix(0, nrow = length(compare$apd_date), ncol =1)
Wo <- 0
for (j in 1:length(compare$apd_date)) {
...
# Model 4 Predictions
m4_pred <- matrix(0, nrow = length(compare$apd_date), ncol = length(fields))
for (k in 1:length(fields)) {
  a <- m4_coef[k,1]
  b <- m4_coef[k,2]
  c <- m4_coef[k,3]
...
# Model 5 Predictions
m5_pred <- matrix(0, nrow = length(compare$apd_date), ncol = length(fields))
for (k in 1:length(fields)) {
  Wo <- 0
  for (j in 1:length(compare$apd_date)) {
...
# Model 6 Predictions
m6_pred <- matrix(0, nrow = length(compare$apd_date), ncol = length(fields))
for (k in 1:length(fields)) {
  a <- m6_coef[k,1]
  b <- m6_coef[k,2]
  c <- m6_coef[k,3]
...
# Model 7 Predictions
m7_pred <- matrix(0, nrow = length(compare$apd_date), ncol = length(fields))
for (k in 1:length(fields)) {
  a <- m7_coef[k,1]
  b <- m7_coef[k,2]
  c <- m7_coef[k,3]
...
# Model 8 Predictions
m8_pred <- matrix(0, nrow = length(compare$apd_date), ncol =1)
Wo <- 0
for (j in 1:length(compare$apd_date)) {
...
# Plot output
plot(compare$apd_date, compare$APDs_approved, type = "l", lty = 1, lwd = 2, xlab = "Date", ylab = "Number of APDs Approved",
     main = "Comparison of Actual vs. Predicted APD Approvals")
lines(compare$apd_date, m1_pred, col = "red")
lines(compare$apd_date, m2_pred, col = "blue")
lines(compare$apd_date, m3_pred, col = "green")
lines(compare$apd_date, m4_pred, col = "orange")
lines(compare$apd_date, m5_pred, col = "brown")
lines(compare$apd_date, m6_pred, col = "yellow")
lines(compare$apd_date, m7_pred, col = "purple")
lines(compare$apd_date, m8_pred, col = "gray")
