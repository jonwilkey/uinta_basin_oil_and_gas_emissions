...
load(file.path(data_root, "mc_prod.oil.rda"))
...
# save(file=file.path(data_root, "mc_prod.oil.rda"), list=c("prod.oil"))
...
#-------------------------------------------------------------------------------
keeps <- c("prod_date", "w_well_type", "p_oil_prod", "p_rpt_period")
...
pdf(file = paste("conventional_v5 Results.pdf"))
