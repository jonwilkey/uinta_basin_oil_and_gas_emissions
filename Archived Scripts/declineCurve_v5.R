#-------------------------------------------------------------------------------
# declineCurve_v5.R (DOGM Decline Curve Fitting)
# Version 5
# 02/18/14
# Jon Wilkey
#-------------------------------------------------------------------------------
...
# Reject values < 0 in dataset to fit with log-normal distribution rules
for (i in 1:length(coef.hyp.oow)) {
  temp <- coef.hyp.oow[[i]]
  temp <- temp[which(temp[,1] > 0 & temp[,1] < 100*summary(temp[,1])[5] &
                     temp[,2] > 0 & temp[,2] < 100*summary(temp[,2])[5] &
                     temp[,3] > 0 & temp[,3] < 100*summary(temp[,3])[5]),]
  coef.hyp.oow[[i]] <- temp
}

# Find log-normal mean & standard deviation for each field
pdf.lognorm.coef <- matrix(0, nrow = length(coef.hyp.oow)*3, ncol = 2)
for (i in 1:length(coef.hyp.oow)) {
  for (j in 1:3) {
    temp <- coef.hyp.oow[[i]][,j]
    temp <- coef(fitdistr(temp, "log-normal"))
    # Log-normal Mean
    pdf.lognorm.coef[((i*3)-(3-j)),1] <- temp[1]
    # Log-normal standard deviation
    pdf.lognorm.coef[((i*3)-(3-j)),2] <- temp[2]
  }
}
...
hist.list <- list(coef.hyp.oow[[1]][,1], coef.hyp.oow[[1]][,2], coef.hyp.oow[[1]][,3],
                  coef.hyp.oow[[2]][,1], coef.hyp.oow[[2]][,2], coef.hyp.oow[[2]][,3],
                  coef.hyp.oow[[3]][,1], coef.hyp.oow[[3]][,2], coef.hyp.oow[[3]][,3],
                  coef.hyp.oow[[4]][,1], coef.hyp.oow[[4]][,2], coef.hyp.oow[[4]][,3],
                  coef.hyp.oow[[5]][,1], coef.hyp.oow[[5]][,2], coef.hyp.oow[[5]][,3],
                  coef.hyp.oow[[6]][,1], coef.hyp.oow[[6]][,2], coef.hyp.oow[[6]][,3],
                  coef.hyp.oow[[7]][,1], coef.hyp.oow[[7]][,2], coef.hyp.oow[[7]][,3],
                  coef.hyp.oow[[8]][,1], coef.hyp.oow[[8]][,2], coef.hyp.oow[[8]][,3],
                  coef.hyp.oow[[9]][,1], coef.hyp.oow[[9]][,2], coef.hyp.oow[[9]][,3],
                  coef.hyp.oow[[10]][,1], coef.hyp.oow[[10]][,2], coef.hyp.oow[[10]][,3],
                  coef.hyp.oow[[11]][,1], coef.hyp.oow[[11]][,2], coef.hyp.oow[[11]][,3],
                  coef.hyp.oow[[12]][,1], coef.hyp.oow[[12]][,2], coef.hyp.oow[[12]][,3],
                  coef.hyp.oow[[13]][,1], coef.hyp.oow[[13]][,2], coef.hyp.oow[[13]][,3],
                  coef.hyp.oow[[14]][,1], coef.hyp.oow[[14]][,2], coef.hyp.oow[[14]][,3],
                  coef.hyp.oow[[15]][,1], coef.hyp.oow[[15]][,2], coef.hyp.oow[[15]][,3],
                  coef.hyp.oow[[16]][,1], coef.hyp.oow[[16]][,2], coef.hyp.oow[[16]][,3],
                  coef.hyp.oow[[17]][,1], coef.hyp.oow[[17]][,2], coef.hyp.oow[[17]][,3],
                  coef.hyp.oow[[18]][,1], coef.hyp.oow[[18]][,2], coef.hyp.oow[[18]][,3])
...
pdf(file = "PDF results for OOW from declineCurve_v5.pdf")
for (i in 1:length(pdf.plot.list.ow)) {
  ...
  xfit <- seq(min(pdf.plot.list.ow[[i]]$x), max(pdf.plot.list.ow[[i]]$x), length = 100)
  yfit <- dlnorm(xfit, meanlog = pdf.lognorm.coef[i,1], sdlog = pdf.lognorm.coef[i,2])
  lines(xfit, yfit, col = "red")
  legend("topright", c("density() PDF", "Log-normal"), lty = c(1,1), col = c("blue", "red"))
}
dev.off()
