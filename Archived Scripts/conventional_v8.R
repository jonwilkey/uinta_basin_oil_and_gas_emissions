...
# Script Info
#-------------------------------------------------------------------------------
# conventional_v8.R (Conventional Oil and Gas Model)
# Version 8
# 04/29/14
# Jon Wilkey
#-------------------------------------------------------------------------------
...
#
# --- Version 7 ---
# 1. Corrected determination of actual oil and gas production rates to only
#    select production data from Uintah and Duchesne counties.
#
# --- Version 8 ---
# 1. Added back code for determining drilling schedule using regression curves
...
#   temp.fit <- temp
#   for (i in 1:length(field.ow)) {
#     a.fit <- decline.field.ow[which(decline.field.ow[,1] == field.ow[i]),4]
#     b.fit <- decline.field.ow[which(decline.field.ow[,1] == field.ow[i]),5]
#     c.fit <- decline.field.ow[which(decline.field.ow[,1] == field.ow[i]),6]
#     for (j in 1:length(all_months)) {
...
#           oil <- a*(1+b*c*t)^(-1/b)
#           oil.fit <- a.fit*(1+b.fit*c.fit*t)^(-1/b.fit)
#           
...
#             temp.fit[g,i] <- temp.fit[g,i] + oil.fit[t.step]
#             t.step <- t.step + 1
#           }
#         }
#       }
...
#   prod.oil.fit[,h] <- rowSums(temp.fit)
#   
#   # Update progress bar
#   Sys.sleep(0.1)
...
#   temp.fit <- temp
#   for (i in 1:length(field.gw)) {
#     a.fit <- decline.field.gw[which(decline.field.gw[,1] == field.gw[i]),7]
#     b.fit <- decline.field.gw[which(decline.field.gw[,1] == field.gw[i]),8]
#     c.fit <- decline.field.gw[which(decline.field.gw[,1] == field.gw[i]),9]
#     for (j in 1:length(all_months)) {
...
#           t <- 1:(length(all_months) - j + 1)
#           gas <- a*(1+b*c*t)^(-1/b)
#           gas.fit <- a.fit*(1+b.fit*c.fit*t)^(-1/b.fit)
#           
...
#             temp.fit[g,i] <- temp.fit[g,i] + gas.fit[t.step]
#             t.step <- t.step + 1
#           }
#         }
#       }
...
#   prod.gas.fit[,h] <- rowSums(temp.fit)
#   
#   # Update progress bar
#   Sys.sleep(0.1)
#   setTxtProgressBar(pb, h)
...
#-------------------------------------------------------------------------------
keeps <- c("prod_date", "w_well_type", "p_oil_prod", "p_gas_prod", "p_rpt_period", "w_county")
poa <- subset(m, select = keeps)
poa <- subset(poa, subset = (w_county == "UINTAH" | w_county == "DUCHESNE"))
pog <- poa
...
pdf(file.path(plot_root, "conventional_v8 Results.pdf"))
...
lines(all_months, prod.oil.fit,      col = "purple")
legend("topleft",
       c("Actual", "MC 95%", "MC 50%", "MC 5%", "Fit"),
       lty = c(1,1,1,1,1),
       col = c("black", "blue", "green", "red", "purple"))
# Plot of Predicted vs Actual Gas Production
plot(all_months, percentile.gw[,3],
     type = "l",
     lty = 1,
     col = "blue",
     xlab = "Date",
...
lines(all_months, prod.gas.actual,   col = "black")
lines(all_months, prod.gas.fit,      col = "purple")
legend("topleft",
       c("MC 95%", "MC 50%", "MC 5%", "Actual", "Fit"),
       lty = c(1,1,1,1,1),
       col = c("blue", "green", "red", "black", "purple"))
dev.off()

# Plot GHG Emissions to PDF
pdf(file.path(plot_root, "conventional_v5 GHG Emissions.pdf"))
# Plot of Predicted vs Actual Oil Production
