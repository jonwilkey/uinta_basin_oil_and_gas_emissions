...
for (j in 1:length(field.gw[,1])){
  ...
  sum <- sqldf("select sum(proddata_gas_prod) from ps")
  # Add to list field.ow.api.count
  field.gw.api.count <- rbind(field.gw.api.count, count)
  field.gw.prod.sum <- rbind(field.gw.prod.sum, sum)
}
# Merge list of field numbers and counts of unique API numbers in each field
field.gw <- cbind(field.gw, field.gw.api.count, field.gw.prod.sum)
# Drop fields that fail to reach convergence
...
# write.excel(field.ow[,2]/sum(field.ow.api.count))
# write.excel(field.ow[,3]/sum(field.ow.prod.sum[,1]))
...
# write.excel(field.gw[,2]/sum(field.gw.api.count))
# write.excel(field.gw[,3]/sum(as.numeric(field.gw.prod.sum[,1])))
...
# ------------------------------------------------------------------------------
# Export *.rda files for use in other R scripts
# ------------------------------------------------------------------------------

# Oil Fields Selection (Criteria: >=95% of all wells & all production in Basin)
decline.field.ow <- cbind(field.ow, coef.hyp.oow, coef.hyp.gow)
colnames(decline.field.ow) <- c("field", "well.count", "prod",
                                "hyp.oow.alpha","hyp.oow.theta","hyp.oow.delta",
                                "hyp.gow.alpha","hyp.gow.theta","hyp.gow.delta")
decline.field.ow <- subset(decline.field.ow,
                           field.ow[,2]/sum(field.ow.api.count) >= 0.0065 |
                           field.ow[,3]/sum(as.numeric(field.ow.prod.sum[,1])) >= 0.0057)
save(decline.field.ow, file = "decline_field_ow.rda")
# Gas Fields Selection (Criteria: >=95% of all wells & all production in Basin)
decline.field.gw <- cbind(field.gw, coef.hyp.ogw, coef.hyp.ggw)
colnames(decline.field.gw) <- c("field", "well.count", "prod",
                                "hyp.ogw.alpha","hyp.ogw.theta","hyp.ogw.delta",
                                "hyp.ggw.alpha","hyp.ggw.theta","hyp.ggw.delta")
decline.field.gw <- subset(decline.field.gw,
                           field.gw[,2]/sum(field.gw.api.count) >= 0.0053 |
                           field.gw[,3]/sum(as.numeric(field.gw.prod.sum[,1])) >= 0.0078)
save(decline.field.gw, file = "decline_field_gw.rda")