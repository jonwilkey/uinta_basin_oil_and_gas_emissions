...
pwd.drop <- "C:/Users/Jon Wilkey/Google Drive/UBOG_PF/"
...
path$raw  <-    paste(pwd.drop,  "Raw Data",            sep = "")
path$data <-    paste(pwd.drop,  "Prepared Data",       sep = "")
path$look <-    paste(pwd.drop,  "Lookup Tables",       sep = "")
path$plot <-    paste(pwd.drop,  "Plots",               sep = "")
path$work <-    paste(pwd.git,   "ub_o-g_emissions_v2", sep = "")
path$fun  <-    paste(path$work, "/Functions",          sep = "")
path$plotfun <- paste(path$work, "/Functions/Plotting", sep = "")
...
flst <- file.path(path$fun, c("EPFsim.R",
                              "EIAsim.R",
                              "drillsim.R",
                              "priorProd.R",
                              "priorInfo.R",
...
# 2.6 ARIMA Fit Update ----------------------------------------------------

# Run function if opt$GBMfit.update flag is set to "TRUE"
if(opt$ARIMAfit.update == TRUE) {

    # Source function to load
    source(file.path(path$fun, "ARIMAfitUpdate.R"))

    # Function call
    ARIMAfitUpdate(path =     path,
                   eia.hp =   eia.hp,
                   tstart =   opt$ARIMA.tstart,
                   tstop =    opt$ARIMA.tstop,
                   ver =      opt$file_ver,
                   opdq =     opt$ARIMA.opdq,
                   gpdq =     opt$ARIMA.gpdq,
                   plotFlag = opt$ARIMA.plotFit)
}

# Load oil and gas ARIMA fits (op.ARIMA and gp.ARIMA)
load(file.path(path$data, paste("ARIMAfit_", opt$file_ver, ".rda", sep = "")))
...
# Run EPFsim to get price paths for oil and gas
op <- EPFsim(epf.type = opt$epf.type$oil, product  = "oil")
gp <- EPFsim(epf.type = opt$epf.type$gas, product  = "gas")
